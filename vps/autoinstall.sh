...
mv ~/sg01.oneserver.xyz /etc/letsencrypt/live/sg01.hostervpn.com
...
mv ~/bash/menu /usr/bin/menu
chmod +x /usr/bin/menu
mv ~/bash/create.sh /usr/bin/create.sh
chmod +x /usr/bin/create.sh
mv ~/bash/del-exp.sh /usr/bin/del-exp.sh
chmod +x /usr/bin/del-exp.sh
mv ~/bash/user-delete.sh /usr/bin/user-delete.sh
chmod +x /usr/bin/user-delete.sh
mv ~/bash/menu /usr/bin/user-extend.sh
chmod +x /usr/bin/user-extend.sh
mv ~/bash/user-login.sh /usr/bin/user-login.sh
chmod +x /usr/bin/user-login.sh
mv ~/bash/user-list.sh /usr/bin/user-list.sh
...
mv ~/services/rc-local.service /etc/systemd/system/rc-local.service
systemctl enable rc-local.service
service rc-local restart

# install banner
mv ~/banner.html /etc/issue.net
...
mv ~/shadowsocks/config.json /etc/shadowsocks-libev/config.json
service shadowsocks-libev restart

# install cloak
cd
chmod +x ~/app/Cloak-Installer.sh
cd app
./Cloak-Installer.sh
cd
wget https://github.com/cbeuw/Cloak/releases/download/v2.2.1/ck-server-linux-amd64-2.2.1
mv ~/ck-server-linux-amd64-2.2.1 /usr/bin/ck-server
chmod +x /usr/bin/ck-server
mv ~/services/cloak.service /etc/systemd/system/cloak.service
...
# obfs-tls
cd
mv ~/shadowsocks/obfs-tls.json /etc/shadowsocks-libev/obfs-tls.json
mv ~/services/obfs-tls.service /etc/systemd/system/obfs-tls.service
systemctl enable obfs-tls.service
service obfs-tls restart

# obfs-http
mv ~/shadowsocks/obfs-http.json /etc/shadowsocks-libev/obfs-http.json
mv ~/services/obfs-http.service /etc/systemd/system/obfs-http.service
systemctl enable obfs-http.service
service obfs-http restart

# ss-v2ray
mv ~/shadowsocks/v2ray.json /etc/shadowsocks-libev/v2ray.json
mv ~/services/ss-v2ray.service /etc/systemd/system/ss-v2ray.service
systemctl enable ss-v2ray.service
service ss-v2ray restart

# install openvpn
cd
chmod +x ~/app/openvpn-install.sh
cd app
./openvpn-install.sh

#install wireguard
cd
chmod +x ~/app/wireguard-install.sh
cd app
