		...
		if [[ "$OPTION" == "y" ]] || [[ "$OPTION" == "Y" ]]; then
			...
			*)
			...
			if [[ "$ckmethod" == "shadowsocks" ]]; then
				...
				cipher=$(jq -r '.method' <'/etc/shadowsocks-libev/cloak.json')
				Password=$(jq -r '.password' <'/etc/shadowsocks-libev/cloak.json')
				ckuid="$ckbuid"
				ShowConnectionInfo
			fi
...
if [[ "$SHADOWSOCKS" == true ]]; then
	...
	#Config shadowsocks
	rm -f /etc/shadowsocks-libev/cloak.json
	echo "{
    \"server\":\"127.0.0.1\",
    \"server_port\":$SS_PORT,
    \"password\":\"$Password\",
    ...
    \"method\":\"$cipher\",
    \"nameserver\":\"$ss_dns\"
}" >>/etc/shadowsocks-libev/cloak.json
	systemctl daemon-reload
